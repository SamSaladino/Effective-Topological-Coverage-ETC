...
Provides:
- G_string_of_pearls: build the 'string of pearls' graph used in the notebook.
- connect_components: lightly connect components so a graph is connected (deterministic with seed).
- smart_layout: pick a layout depending on graph size.
- show_graph: quick visualization for a single graph.
- show_highlighted_subsets: visualize one or more subsets (highlighted nodes) with a customizable legend.

These functions mirror the logic used in the notebooks so they can be reused programmatically.
"""
...
def G_string_of_pearls(m: int = 7, clique: int = 8) -> nx.Graph:
    """Create a "string of pearls" graph: m cliques of size `clique` connected in a path.

    Returns a graph with integer node labels (0..n-1) ordered sorted.
    """
...
def show_graph(G: nx.Graph, title: Optional[str] = None, node_size: int = 8, ax: Optional[plt.Axes] = None):
...
def show_highlighted_subsets(
    ...
    - subsets: sequence of iterables of node ids (int or str) to highlight. One subplot per subset.
    - titles: optional titles per subplot. If missing, simple "Subset i" labels are used.
    - colors: optional colors per subplot. Defaults to a colorblind-friendly pair repeated.
    - legend_labels: optional labels for legend; must match the meaning of the highlighted nodes.
    - pos: optional layout dictionary (node->(x,y)). If None, smart_layout is computed once.
    ...
    for ax, subset, title, color in zip(axes, subsets, titles, colors):
        ax.set_title(title)
        ax.axis("off")
        nx.draw_networkx_edges(G, pos, ax=ax, width=edge_width, alpha=edge_alpha, edge_color=edge_color)

        subset_set = set(subset)
        background = [n for n in G.nodes() if n not in subset_set]
        nx.draw_networkx_nodes(G, pos, nodelist=background, node_color="#DDDDDD", node_size=node_size_bg, linewidths=0, ax=ax)
...
def get_graphs_list(seed: int = 2):
    ...
    graphs = [
        ...
        ("Core-Periphery", connect_components(nx.stochastic_block_model([80, 220], [[0.18, 0.05],[0.05, 0.01]], seed=seed), seed=seed)),
        ("Erdos-Renyi(n=200,p=0.05)", connect_components(nx.erdos_renyi_graph(200, 0.05, seed=seed), seed=seed)),
        ("Random Partition (communities)", connect_components(nx.random_partition_graph([15, 25, 20], 0.8, 0.02, seed=seed), seed=seed)),
        ...
        ("WS beta=0.0", connect_components(nx.watts_strogatz_graph(150, 8, 0.0, seed=seed), seed=seed)),
        ("WS beta=0.2", connect_components(nx.watts_strogatz_graph(150, 8, 0.2, seed=seed), seed=seed)),
        ("WS beta=1.0", connect_components(nx.watts_strogatz_graph(150, 8, 1.0, seed=seed), seed=seed)),
        ("BA(n=400,m=2)", nx.barabasi_albert_graph(100, 2, seed=seed)),
        ("Powerlaw-Cluster", connect_components(nx.powerlaw_cluster_graph(100, 2, 0.3, seed=seed), seed=seed)),
        ("d-Regular(n=400,d=6)", connect_components(nx.random_regular_graph(6, 100, seed=seed), seed=seed)),
        ("Geometric(n=400,r=0.07)", connect_components(nx.random_geometric_graph(100, 0.07, seed=seed), seed=seed)),
